...
import { ProjectNotesBoard } from "@/components/ProjectNotesBoard";
...
export default function ProjectsPage() {
    ...
    return (
        <main className={`relative min-h-[100dvh] w-full flex flex-col items-center justify-start transition-colors duration-700 bg-transparent`}>
            {/* Background Layers (for Experiments hero section) */}
            ...
            <div className="w-full relative z-10 flex flex-col pt-0 pb-0">
                {/* Curiosity / Experiments Section ("a place to show what i make") */}
                <div className="w-full -mt-4 sm:-mt-8">
                    <ExperimentsSection isDark={isDark} />
                </div>

                {/* Projects Notes Board (has its own background) */}
                <ProjectNotesBoard isDark={isDark} />
            </div>
        </main>
    );
}
